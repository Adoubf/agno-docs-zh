...
以下智能体将使用 AWS Lambda 列出我们 AWS 账户中的所有 Lambda 函数，并调用特定的 Lambda 函数。
...
| `invoke_function` | 调用特定的 Lambda 函数并可选携带载荷。接收 `function_name` 和可选的 `payload` 参数。 |

## 开发者资源

- 查看 [工具](https://github.com/agno-agi/agno/blob/main/libs/agno/agno/tools/aws_lambda.py)
